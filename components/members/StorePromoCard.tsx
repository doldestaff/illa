...
export default function StorePromoCard() {
    ...
    return (
        <Link href="/descontos" className="block group relative">
            {/* PERF: Replaced motion.div with CSS animation for entrance */}
            <div
                className="anim-fade-in-up relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-900/30 via-fuchsia-900/10 to-transparent border border-white/10 shadow-2xl group-hover:shadow-purple-500/20 transition-all duration-500 backdrop-blur-2xl active:scale-[0.98]"
            >
                {/* Background Effects */}
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay pointer-events-none" />

                {/* Smokey Glowing Stained Glass — PERF: CSS-only on mobile, framer-motion on desktop */}
                {isMobile ? (
                    <>
                        {/* Static gradients on mobile — no JS animation loops */}
                        <div
                            ...
                            style={{ animation: 'glow-pulse-soft 8s ease-in-out infinite', opacity: 0.4 }}
                        />
                        <div
                            className="absolute -bottom-[40%] -left-[20%] w-[120%] h-[120%] bg-pink-500/20 rounded-full blur-[100px] pointer-events-none mix-blend-screen opacity-30"
                        />
                    </>
                ) : (
                    <>
                        {/* Desktop: Keep richer animations (no scroll impact on desktop) */}
                        <div
                            className="absolute -top-[40%] -right-[20%] w-[120%] h-[120%] bg-purple-500/20 rounded-full blur-[100px] pointer-events-none mix-blend-screen"
                            style={{ animation: 'glow-pulse-soft 8s ease-in-out infinite' }}
                        />
                        <div
                            ...
                            style={{ animation: 'glow-pulse-soft 10s ease-in-out infinite 1s' }}
                        />
                    </>
                )}
                ...
                <div className="relative z-10 p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 overflow-hidden">
                    ...
                    <div className="flex flex-col items-center sm:items-start text-center sm:text-left space-y-3 w-full">
                        ...
                        {/* Novidade Badge — PERF: CSS pulse instead of motion.div */}
                        <div
                            ...
                            style={{ animation: isMobile ? 'none' : 'glow-pulse-soft 2s ease-in-out infinite' }}
                        >
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        ...
                        <div className="relative w-16 h-16 sm:w-20 sm:h-20 flex items-center justify-center">
                            ...
                            <div className="relative w-full h-full bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-2xl flex items-center justify-center backdrop-blur-xl shadow-inner group-hover:-translate-y-2 transition-transform duration-500 group-hover:border-purple-500/50">
                                ...
                                {/* Floating Badge — PERF: removed animate-bounce on mobile */}
                                <div className={`absolute -top-2 -right-2 bg-amber-400 text-black text-[10px] font-black px-1.5 py-0.5 rounded-md shadow-lg rotate-12 ${isMobile ? '' : 'animate-bounce'}`}>
                                    %
                                </div>
                            </div>
                        </div>
